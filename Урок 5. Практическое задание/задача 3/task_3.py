...
ru_dict = {"0": "Ноль", "1": "Один", "2": "Два", "3": "Три", "4": "Четыре",
           "5": "Пять", "6": "Шесть", "7": "Семь", "8": "Восемь", "9": "Девять"}

numbers_ru = []

try:
    with open(IN_FILE, encoding="utf-8") as f_en:
        for line_en in f_en:
            num = line_en.split(" - ")[1].rstrip()
            numbers_ru.append(f"{ru_dict.get(num)} - {num}")
except IOError:
    print(f"Произошла ошибка чтения файла: {IN_FILE}!")

try:
    with open(OUT_FILE, "w", encoding="utf-8") as f_ru:
        print("\n".join(numbers_ru), file=f_ru)
except IOError:
    print(f"Произошла ошибка записи файла {OUT_FILE}!")
...
try:
    ...
    print(f"Произошла ошибка чтения файла {OUT_FILE}!")
